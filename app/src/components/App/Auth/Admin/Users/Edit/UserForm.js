...
const UserForm = ({ initialData = {}, disabled, onSubmit, label }) => {
    ...
    return (
        <form noValidate={true} method="patch" onSubmit={handleSubmit(handleData)} className="p-12 pt-0 md:p-18 rounded-xl flex flex-col">
              <div className="flex items-cente flex-col justify-between text-lg  sm:flex-row">
                <div className='w-full sm:w-5/12 mb-6'>
                  ...
                  <Input disabled={disabled} className="border mt-3 rounded-lg pl-6 md:py-2 focus:outline-none w-full"  placeholder='Name' name="name" value={values.name} onChange={handleChange} />
                </div>
                <div className='w-full sm:w-5/12 mb-6'>
                  <label htmlFor="surname" className='w-6/12'>{t('fields.surname')}</label>
                  <Input disabled={disabled} className="border mt-3 rounded-lg pl-6 md:py-2 focus:outline-none w-full"  placeholder='Surname' name="surname" value={values.surname} onChange={handleChange} />
                </div>
              </div>
              <div className="flex items-center text-lg mb-6 ">
                <div className='w-full'>
                  <label htmlFor="email" className='w-full'>{t('fields.email')}</label>
                  <Input disabled={disabled} type='email' className="border mt-3 rounded-lg pl-6 md:py-2 focus:outline-none w-full"  placeholder='Email' name="email" value={values.email} onChange={handleChange} />
                </div>
              </div>
              <div className="flex items-center text-lg mb-6 ">
                <div className='w-full'>
                  ...
                  <select onChange={handleChange} name="role" id="role" className="border mt-3 rounded-lg pl-6 md:py-2 focus:outline-none w-full">
                    ...
                    {Object.values(UserRoles).map((role) => (
                      <option key={role} value={role} selected={ role === values.role }>{role}</option>
                    ))}
              ...
              <Button color={'primary'} className={` m-auto bg-blue-500 `} type="submit" disabled={disabled}>
                {/* {isLoading ?
